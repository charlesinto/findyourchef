...
const fetchChefReview = (id) => {
  ...
  axios
    .get(`${baseURL}/chef/reviews/${id}`, data)
    .then((res) => {
...
const addReviewToDOM = () => {
  ...
  const stars =
    (serviceStars + valueStars + cleanlinessStars + locationStars) / 4;
...
const bookChef = (e) => {
  ...
  if (token) {
    if (userData.role === "chef") {
      ...
      const success_url =
        "http://www.thepottersmind.com/fyc/payment_success.html";
      const cancel_url =
        "http://www.thepottersmind.com/fyc/payment_failure.html";
      const data = {
        bookersID,
        recipeID,
        amount,
      ...
      axios
        .post(
          `https://thepotters-apiv2.herokuapp.com/api/v1/recipe/book?type=chef`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then((res) => {
          ...
          var stripe = Stripe(
            "pk_test_51I1DVaBUbM1BcmYudXugXJGteb66XL2NS7IMWHRDCLcNwWbwdOgJJD4b1hyo2DUaNgH4HxGn5RamRO4djeX4usXg00rM6sJg8k"
          );
...
const loadAllRecipes = () => {
  ...
  axios
    .post(`${baseURL}/chef/recipe/list?status=active&page=${page}`, data, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
      ...
      axios
        ...
        .then((res) => {
          ...
          localStorage.setItem(
            "fyc-bookmark-recipe-id",
            res.data.payload.data._id
          );
        })
        .catch((err) => {
          if (err.response && err.response.data) {
